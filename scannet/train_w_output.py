#train_w_output.py
#Trains Pointnet++ Semantic Segmentation on Scannet data, then tests using the Scannet data test set and outputs
#results and point clouds of predicted (ending with pred.txt) and ground truth (ending with gt.txt) segmentations. 
...
parser.add_argument('--log_dir', default='pointcloud_log2', help='Log dir [default: log]')
parser.add_argument('--num_point', type=int, default=4000, help='Point Number [default: 8192]')#larger values here take larger samples from the image but can crash the program.
parser.add_argument('--max_epoch', type=int, default=201, help='Epoch to run [default: 201]')
parser.add_argument('--batch_size', type=int, default=1, help='Batch Size during training [default: 32]') #left as 1 because larger numbers can crash the program.
...
def train():
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            if epoch%5==0:
                #out_filename = str(epoch) + 'predfinal'
                out_filename = 'pointcloud'
                out_filename = os.path.join(DUMP_DIR, out_filename)
                print(out_filename)
                acc = eval_one_epoch(sess, ops, test_writer, out_filename)
                acc = eval_whole_scene_one_epoch(sess, ops, test_writer, out_filename)
            if acc > best_acc:
                best_acc = acc
...
def eval_one_epoch(sess, ops, test_writer, out_filename):
    ...
    for batch_idx in range(num_batches):
        ...
        for i in range(NUM_POINT):
            ...
            fout.write('%f %f %f %d %d %d\n' % (batch_data[0,i,0], batch_data[0,i,1], batch_data[0,i,2], color[0], color[1], color[2]))
            gtfout.write('%f %f %f %d %d %d\n' % (batch_data[0,i,0], batch_data[0,i,1], batch_data[0,i,2], gtcolor[0], gtcolor[1], gtcolor[2]))
...
def eval_whole_scene_one_epoch(sess, ops, test_writer, out_filename):
    """ ops: dict mapping from string to tf ops """
    global EPOCH_CNT
    is_training = False
    ...
    for batch_idx in range(num_batches):
        ...
        fname = out_filename + str(batch_idx) + "_pred.txt";
        gt_fname = out_filename + str(batch_idx) + "_gt.txt";
        fout = open(fname, 'a') #open files with append to add more data onto the image sample created by eval_one_epoch
        gtfout = open(gt_fname, 'a')
        for i in range(NUM_POINT):
            color = label2color[pred_val[0,i]]
            gtcolor = label2color[batch_label[0,i]]
            fout.write('%f %f %f %d %d %d\n' % (batch_data[0,i,0], batch_data[0,i,1], batch_data[0,i,2], color[0], color[1], color[2]))
    ...
    for i in range(NUM_POINT):
        color = label2color[batch_label[0,i]]
        fout.write('%f %f %f %d %d %d\n' % (aug_data[0,i,0], aug_data[0,i,1], aug_data[0,i,2], color[0], color[1], color[2]))
